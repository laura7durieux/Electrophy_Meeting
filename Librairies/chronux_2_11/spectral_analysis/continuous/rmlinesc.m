...
if nargin < 5; f0=[]; end;
if isempty(f0) && user_specified_pval==1; p=p/N; 
%%%% ADDED BY N. TOTAH
...
if nargout==0 || strcmp(params.plt,'y'); 
   figure;
   [S1,f]=mtspectrumc(detrend(data),params);
   figure; plot(f,10*log10(S1));xlabel('frequency Hz'); ylabel('Spectrum dB'); title('Original spectrum'); xlim([0 100])
...
figure; plot(f,10*log10(S2));xlabel('frequency Hz'); ylabel('Spectrum dB'); title('Cleaned spectrum'); xlim([0 100])
end;
data=datan;   
